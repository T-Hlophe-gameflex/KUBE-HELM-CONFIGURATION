...
if [[ -z "$ZONE_NAME" ]]; then
  ...
  if [[ -z "$CF_TOKEN" ]]; then
  read -r -s -p "Enter Cloudflare API token (will not be echoed): " CF_TOKEN
  echo
  if [[ -z "$CF_TOKEN" ]]; then
...
if [[ -n "$DELETE_ID" ]]; then
  ...
  if [[ "$SKIP_CONFIRM" != true ]]; then
    read -r -p "Are you sure you want to DELETE record $DELETE_ID from zone $ZONE_NAME? (y/N) " resp
    case "$resp" in
      [yY][eE][sS]|[yY]) :;;
...
if [[ "$CREATE_TEST" == true ]]; then
  ...
  if [[ "$SKIP_CONFIRM" != true ]]; then
    read -r -p "Create test ${RECORD_TYPE} record name 'cf-inspect-${ZONE_NAME}'? (y/N) " resp
    case "$resp" in
      [yY][eE][sS]|[yY]) :;;
  ...
  if [[ "$CREATE_TEST" == true ]]; then
    ...
    # Use jq to build the payload first; if that fails, fallback to printf
    payload=$(jq -n --arg t "$RECORD_TYPE" --arg n "$test_name" --arg c "$TEST_CONTENT" --argjson ttl "${TEST_TTL}" --arg proxied "${TEST_PROXIED}" \
      '{type:$t, name:$n, content:$c, ttl:$ttl, proxied: (if ($proxied == "true") then true elif ($proxied == "false") then false else false end) }' 2>/dev/null) || true

    # If building with jq above failed (jq oddness), fallback to simple payload built safely
    if [[ -z "$payload" || "$payload" == "null" ]]; then
      payload=$(printf '{"type":"%s","name":"%s","content":"%s","ttl":%s,"proxied":%s}' "$RECORD_TYPE" "$test_name" "$TEST_CONTENT" "$TEST_TTL" "$TEST_PROXIED")
    fi
