...
show_cleanup_config() {
    log_header "📋 Cleanup Configuration"
    ...
    case "$CLEANUP_MODE" in
        complete)
            ...
            echo "  🗑️  ELK Stack (Elasticsearch, Kibana, Logstash, Filebeat)"
            echo "  🗑️  Application Services (PostgreSQL, Order Service, User Service)"
            echo "  🗑️  Associated PVCs and ConfigMaps"
            ;;
        elk-only)
            echo "  🗑️  ELK Stack (Elasticsearch, Kibana, Logstash, Filebeat)"
            echo "  🗑️  Associated PVCs and ConfigMaps"
        ...
        apps-only)
            ...
            ;;
        cluster)
            echo "  🗑️  Entire Kind Kubernetes cluster"
            echo "  🗑️  Docker containers and networks"
            echo "  🗑️  Local cluster configuration"
            ;;
    esac
    echo ""
}
...
# Cleanup ELK stack
cleanup_elk() {
    log_header "🗑️  Cleaning Up ELK Stack"
...
cleanup_apps() {
    log_header "🗑️  Cleaning Up Application Services"
...
# Cleanup MetalLB
cleanup_metallb() {
    log_header "🗑️  Cleaning Up MetalLB"
...
cleanup_cluster() {
    log_header "🗑️  Cleaning Up Entire Cluster"
...
execute_cleanup() {
    log_header "🧹 Starting Infrastructure Cleanup"
...
validate_cleanup() {
    log_header "🔍 Post-Cleanup Validation"
...
show_completion_info() {
    ...
    case "$CLEANUP_MODE" in
        cluster)
            ...
            echo "  🔄 To create a new cluster: make setup-cluster"
            ;;
        *)
            echo "  ✅ Selected components removed"
    ...
    echo -e "${CYAN}🧹 Additional Cleanup (if needed):${NC}"
...
# Main execution
main() {
    log_header "🧹 Infrastructure Cleanup Script"
    ...
    if [[ "$FORCE" != "true" ]]; then
        echo -e "${RED}⚠️  WARNING: This will permanently delete selected infrastructure components!${NC}"
